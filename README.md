...
数据完全存在本机，不需要账号、不上云、不依赖联网（仅汇率刷新会优先调用中国外汇交易中心公开数据，并以 Frankfurter 公共 API 作为备用源）。
...
- **风险与配置**：资产波动率柱图、相关性矩阵热力图、带当前/目标/增减金额的再平衡提醒
...
- 资产可设置目标占比；填写 `0%` 或留空表示不参与再平衡，且不会进入其他资产的比例分母
- 按成员分组渲染，每组组首显示参与项目标占比合计与达标状态；合计不是 `100%` 时不生成调仓金额
- 支持用户主动点击「一键归一化」，仅把正目标占比项目等比例缩放到合计 `100%`，不会重新纳入 `0%` / 空目标项目
...
- 设置基准币种（默认 CNY）
- 查看当前时区与汇率来源（中国外汇交易中心 + Frankfurter）
- 配置再平衡阈值
- 导出 / 导入迁移包做备份与恢复
