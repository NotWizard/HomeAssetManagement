...
export function AnalyticsDateRangePicker({
  ...
  const [visibleMonth, setVisibleMonth] = useState(() => startOfMonth(parseDateValue(endDate) ?? new Date()));
  const rangeSummary = buildRangeSummary(startDate, endDate);

  const activeValue = openField === 'start' ? startDate : endDate;
  const activeMin = openField === 'end' ? startDate : undefined;
  const activeMax = openField === 'start' ? endDate : undefined;
  ...
  useEffect(() => {
    ...
    const initialDate = parseDateValue(openField === 'start' ? startDate : endDate) ?? new Date();
    setVisibleMonth(startOfMonth(initialDate));
  }, [openField, startDate, endDate]);
  ...
  const handleFieldToggle = (field: PickerField) => {
    ...
    const initialDate = parseDateValue(nextField === 'start' ? startDate : endDate) ?? new Date();
    setVisibleMonth(startOfMonth(initialDate));
  };
