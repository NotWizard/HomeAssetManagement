...
test('分析看板使用新的时间区间组件，并让整块日期卡片可点击', () => {
  ...
  assert.match(pickerSource, /inputRef=\{startInputRef\}/);
  assert.match(pickerSource, /inputRef=\{endInputRef\}/);
});
...
test('分析看板时间区间组件应作为内嵌式单控件，而不是带边框阴影的独立卡片', () => {
  ...
  assert.match(pickerSource, /bg-slate-100\/80 p-2/);
  assert.match(pickerSource, /rounded-\[18px\] bg-white\/80 px-1/);
