...
test('桌面服务层会优先通过 bridge.api 发起 JSON 请求', () => {
  const apiClientSource = readFrontendFile('src/services/apiClient.ts');

  assert.match(apiClientSource, /getDesktopBridge\(\)/);
  assert.match(apiClientSource, /desktopBridge\.api\.requestJson\(/);
});

test('桌面模式会通过 bridge.api 上传表单，而不是直接拼接 localhost 地址', () => {
  const apiClientSource = readFrontendFile('src/services/apiClient.ts');

  assert.match(apiClientSource, /desktopBridge\.api\.postForm\(/);
  assert.match(apiClientSource, /serializeFormData/);
});

test('迁移包与错误报告下载在桌面模式下也会通过 bridge.api 走本地 sidecar', () => {
  ...
  assert.match(migrationSource, /desktopBridge\.api\.requestBinary\(/);
  assert.match(migrationSource, /postForm</);

  assert.match(importsSource, /getDesktopBridge\(\)/);
  assert.match(importsSource, /desktopBridge\.api\.requestBinary\(/);
});
